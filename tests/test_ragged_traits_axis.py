...
def _make_records() -> list[TraitRecord]:
    return [
        TraitRecord(
            ...
            trait_id="ENSG00000000003", n=500,
            trait_chr="1", trait_bp=1_500_000,
        ...
        TraitRecord(
            ...
            trait_id="cg00000029", n=800,
            trait_chr="2", trait_bp=5_000_000,
...
def test_write_and_tabix_range(tmp_path):
    ...
    assert {r.trait_id for r in results} == {"ENSG00000000003", "ENSG00000000005"}
...
def test_range_second_chromosome(tmp_path):
    ...
    assert {r.trait_id for r in results} == {"ENSG00000000457", "cg00000029", "ENSG00000000419"}
...
def test_by_trait_id(tmp_path):
    records = _make_records()
    write_traits_axis(tmp_path, records)

    reader = TraitsAxisReader(tmp_path)
    results = reader.by_trait_id("ENSG00000000419")
...
def test_optional_fields_roundtrip(tmp_path):
    ...
    reader = TraitsAxisReader(tmp_path)
    results = reader.by_trait_id("cg00000029")
    assert len(results) == 1
    r = results[0]
...
def test_context_field_roundtrip(tmp_path):
    ...
    reader = TraitsAxisReader(tmp_path)
    results = reader.by_trait_id("ENSG00000000457")
...
def test_all_iterator(tmp_path):
    ...
    # Sorted by chr/bp
    assert all_records[0].trait_chr == "1"
    assert all_records[2].trait_chr == "2"
...
def test_no_tabix_when_positions_missing(tmp_path):
    records = [
        TraitRecord(
            ...
            trait_id="ukb-b-1234", n=5000,
            trait_chr=None, trait_bp=None,
