...
The `tophits` row is a non-empty, analysis-addressed query for
`ENSG00000198502`, limited to its first 10 genome-wide-significant hits in
genomic order. The Ragged top-hit tiers reuse the layout's natural
analysis-major CSR order and add per-tier offsets, so the selected query reads
one contiguous slice. It fell from the previous **146.95 ms** global index read
to **1.71 ms** median (**86× faster**); rebuilding all tiers from 121.9 million
associations took **12.1 seconds**, and the selected result matched the
corresponding global subset.
...
param_rows = [
    ...
    ("Top-hit analysis", sel["top_hit_analysis_id"]),
