...
class CompletionQualityRollup:
    """One Analysis's completion-rollup columns, named rather than positional
    so Dense/Ragged's differing use of ``n_missing_total`` (see
    ``completion_quality_rollup`` below) can't be mixed up by index."""

    median_pearson_r: str
    n_imputed_total: str
    n_missing_total: str
...
def completion_quality_rollup(
    connection: Any, n_analyses: int
) -> dict[int, CompletionQualityRollup]:
    """Per-Analysis completion-rollup columns, rolled up from the LD-block-by-
    Analysis ``completion_quality`` table (issue #22) -- the large,
    fine-grained table stays SQLite-only per ADR 0030; only this rollup
    travels into ``analyses.tsv``. Shared by Dense and Ragged completion
    (issue #70): Dense computes its own ``completion_n_missing_total`` from
    off-panel missingness instead (a different quantity -- variants never
    attempted, not variants attempted-and-failed) and ignores
    ``.n_missing_total`` here; Ragged has no separate off-panel concept, so
    it uses this rollup's value directly.
    """
    ...
    rollup: dict[int, CompletionQualityRollup] = {}
    for idx in range(n_analyses):
        r_values = per_analysis_r.get(idx, [])
        median_r = f"{float(np.median(r_values)):.6g}" if r_values else ""
        rollup[idx] = CompletionQualityRollup(
            median_pearson_r=median_r,
            n_imputed_total=str(per_analysis_imputed.get(idx, 0)),
            n_missing_total=str(per_analysis_missing.get(idx, 0)),
        )
    return rollup
