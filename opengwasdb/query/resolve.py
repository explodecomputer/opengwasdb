...
produce (it isn't part of the alid), so it's resolved via
...
docstring) -- and only when `include_rsid=True`, since that's the one part
of this join that can cost anywhere from milliseconds to tens of seconds on
a large dense query. The analysis side comes from `analyses.all()`, already
fully loaded at store-open (ADR 0030, no extra I/O). Rows are yielded
lazily, so the CLI's CSV writer streams rather than materialising the whole
resolved table.
"""
...
def resolve_rows(
    ...
    *,
    include_rsid: bool = False,
) -> Iterator[dict[str, object]]:
    """Yield one human-readable row per association in `result`.

    Each row carries analysis_id/analysis_label, variant identity
    (chromosome/position/alid/alleles, plus rsid when `include_rsid=True`
    -- "." when the store has none for that variant, matching the Store
    Variant Table's own missing marker), z, se, log10_p, and
    association_status. `rsid` is omitted from the row entirely when
    `include_rsid=False` (the default) rather than filled with a
    placeholder, since resolving it is the expensive part of this join at
    scale and skipping it is the point of the flag.
    """
    ...
    variant_rows = (
        variant_axis.by_indices(variant_index) if include_rsid or identity is None else None
    )
